...
#             nn.Conv3d(n_channels, 3 * n_channels, kernel_size=1),  # (B, 3 * C, H, W, D)
#             Attention(n_heads),
#             zero_init(nn.Conv3d(n_channels, n_channels, kernel_size=1)),
#         )
...
#     def __init__(self,ch_in,ch_out=None,condition_dim=None,dropout_prob=0.0,norm_groups=None):
#         super().__init__()
...
#             nn.GroupNorm(num_groups=norm_groups, num_channels=ch_out), # extra
#             nn.SiLU(),
#             nn.Conv3d(ch_out, ch_out, kernel_size=3, padding=1),  # extra
#         )
...
#             # zero_init(nn.Conv3d(ch_out, ch_out, kernel_size=3, padding=1)),  ####################
#         )
#         if ch_in != ch_out:
...
#             condition = condition[:, :, None, None, None] #2d   ######################
#             h = h + condition
#         h = self.net2(h)
...
#         # Output features are cos and sin of above. Shape (B, 2 * F * C, H, W, D).
...
#         # self.conv_in = nn.Conv3d(total_input_ch, embedding_dim, kernel_size=3, padding=1)
...
#             # zero_init(nn.Conv3d(embedding_dim, input_channels, 3, padding=1)),
#         )
...
#         # print("H ARRAY SHAPE = ",h.shape)
#         #standard UNet from here but with cond at each layer
#         h = self.conv_in(h)  # (B, embedding_dim, H, W, D)
#         hs = []
...
class DownBlock(nn.Module):
    """
    factor : per-axis (LoS, y, z) downsampling stride, e.g. (2,2,2) or (2,1,1).
             Implemented as a strided conv with kernel == stride (clean inverse).
    """
    def __init__(self, resnet_block, factor=(2, 2, 2)):
        super().__init__()
        self.resnet_block = resnet_block
        self.factor = tuple(factor)
        self.down = nn.Conv3d(self.resnet_block.ch_out, self.resnet_block.ch_out,
...
class UpBlock(nn.Module):
    """
    factor : per-axis (LoS, y, z) upsampling stride. MUST match the corresponding
             DownBlock factor so the skip-connection shapes line up.
    """
    def __init__(self, resnet_block, factor=(2, 2, 2)):
        super().__init__()
        self.resnet_block = resnet_block
        self.factor = tuple(factor)
        self.up = nn.ConvTranspose3d(self.resnet_block.ch_out * 2,
                                     self.resnet_block.ch_out,
                                     kernel_size=self.factor, stride=self.factor)
...
class UNetVDM(nn.Module):
    """
    Anisotropic-downsampling VDM UNet.

    down_factors : list of per-block (LoS, y, z) stride tuples, length n_blocks.
        Default reproduces isotropic 2x everywhere. For the anisotropic schedule
        (LoS 1024, transverse 16) example:
            [(2,2,2), (2,1,1), (2,2,2), (2,1,1)]
        gives bottleneck LoS 64, transverse 4x4.
        The up-path automatically mirrors these in reverse order.
    """
    def __init__(
        self,
        input_channels: int = 1,
        ...
        # default: isotropic 2x per block (original behaviour)
        if down_factors is None:
            down_factors = [(2, 2, 2)] * n_blocks
        assert len(down_factors) == n_blocks, \
            f"down_factors len {len(down_factors)} != n_blocks {n_blocks}"
        self.down_factors = [tuple(f) for f in down_factors]

        attention_params = dict(
            n_heads=n_attention_heads,
            n_channels=(2**n_blocks)*embedding_dim,
            norm_groups=norm_groups,
        )
        ...
        if use_fourier_features:
            self.fourier_features = FourierFeatures(first=5.0, last=7.0, step=1)
        ...
        self.conv_in = nn.Conv3d(total_input_ch, embedding_dim, kernel_size=3,
        ...
        for i in range(n_blocks):
            self.down_blocks.append(DownBlock(
                resnet_block=ResnetBlock(ch_in=(dim//2 if i != 0 else dim),
                                         ch_out=dim, **resnet_params),
        ...
        # Up of UNet — mirror the down factors in reverse
        self.up_blocks = nn.ModuleList()
        up_factors = list(reversed(self.down_factors))
        for i in range(n_blocks):
            self.up_blocks.append(UpBlock(
                resnet_block=ResnetBlock(ch_in=dim, ch_out=dim//2, **resnet_params),
                factor=up_factors[i]))
            dim //= 2
    ...
    def forward(self, z, g_t, conditioning=None):
        ...
        g_t = g_t.expand(z_concat.shape[0])
        assert g_t.shape == (z_concat.shape[0],)
